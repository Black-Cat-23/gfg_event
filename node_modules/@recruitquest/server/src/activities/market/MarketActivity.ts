import { Activity, MarketConfig, Submission, Score, DecisionAction, Portfolio } from '@recruitquest/types';
import { repo } from '../../db/repository';
import { timerService } from '../../engine/TimerService';
import { calculatePortfolioValue } from '@recruitquest/activities';
import { Server, Socket } from 'socket.io';

const MASTER_PRICE_LEDGER: Record<string, number[]> = {
...
export class MarketActivityEngine {
  ...
  private teamPortfolios: Map<string, Portfolio> = new Map();
  private assignedCompanies: Map<string, string> = new Map(); // key: teamId, value: companyName
  private previousRanks: Map<string, number> = new Map(); // key: teamId, value: rank
  private currentPhase: 'trading' | 'reveal' = 'trading';

  constructor(activity: Activity, io: Server) {
    this.activity = activity;
    this.config = activity.config as MarketConfig;
    this.io = io;
  }
  ...
  async sendTradingPhase(index: number) {
    ...
    for (const team of teams) {
      ...
      if (!pf) {
        ...
        const r0Price = MASTER_PRICE_LEDGER[assignedComp.name][0];
        const holdings: Record<string, number> = {};
        for (const c of companies) holdings[c.name] = c.name === assignedComp.name ? 1 : 0;
        pf = { cash: 10000 - r0Price, holdings, totalValue: 10000 };
        this.assignedCompanies.set(team.id, assignedComp.name);
        this.teamPortfolios.set(team.id, pf);
      }

      const sub = await repo.getSubmission(this.activity.id, team.id);
      let submittedDecisions: Record<string, DecisionAction> | undefined = undefined;
  ...
  sendScenarioToSocket(socket: Socket, teamId?: string) {
    const rounds = this.config.rounds;
  ...
  async onSubmit(teamId: string, payload: { roundIndex: number; decisions: Record<string, DecisionAction> }) {
    ...
    if (!pf) return { success: false, error: 'Team portfolio not initialized' };

    // Validate decisions: single-share limit & cash/holding constraints
    const sanitizedDecisions: Record<string, DecisionAction> = {};
  ...
  private async handleTradingPhaseTimeout(index: number) {
    await this.closeTradingPhaseAndReveal(index);
  }

  async closeRoundAndAdvance(index: number) {
    if (this.currentPhase === 'trading') {
      await this.closeTradingPhaseAndReveal(index);
    } else {
  ...
  async closeTradingPhaseAndReveal(index: number) {
    timerService.clearTimer(this.activity.id);
    ...
    const impacts: { company: string; pctChange: number; oldPrice: number; newPrice: number; isBankrupt?: boolean }[] = [];
    ...
    for (const team of teams) {
      ...
      if (!pf) {
        const holdings: Record<string, number> = {};
        for (const c of this.config.companies) holdings[c.name] = 0;
        pf = { cash: 10000, holdings, totalValue: 10000 };
      }

      const teamRounds = subMap.get(team.id) || {};
      const roundSubmission = teamRounds[index];
      const decisions: Record<string, DecisionAction> = roundSubmission?.decisions || {};
  ...
  private async handleRevealPhaseTimeout(index: number) {
    timerService.clearTimer(this.activity.id);
  ...
  private async onMarketFinished() {
    timerService.clearTimer(this.activity.id);
